...
<div class="sbox animated fadeInRight">
	...
	<div class="sbox-content"> 	
	    <div class="toolbar-line ">
			@if($access['is_add'] ==1)
	   		<a href="{{ URL::to('outbound/add?md='.$masterdetail["filtermd"].$trackUri) }}" class="tips btn btn-sm btn-white"  title="{{ Lang::get('core.btn_create') }}">
			...
			<a href="{{ URL::to('outbound/download?md='.$masterdetail["filtermd"].$trackUri) }}" class="tips btn btn-sm btn-white" title="{{ Lang::get('core.btn_download') }}">
	...
	{{ $details }}
	 ...
	 {{ Form::open(array('url'=>'outbound/destroy/', 'class'=>'form-horizontal' ,'id' =>'SximoTable' )) }}
            ...
            @foreach ($rowData as $row)
                <tr>
				 ...
				 <td>
						...
						<a href="{{ URL::to('outbound/show/'.$id.'?md='.$masterdetail["filtermd"].$trackUri)}}" class="tips btn btn-xs btn-white" title="{{ Lang::get('core.btn_view') }}"><i class="fa  fa-search"></i></a>
						@endif
						@if($access['is_edit'] ==1)
						<a  href="{{ URL::to('outbound/add/'.$id.'?md='.$masterdetail["filtermd"].$trackUri)}}" class="tips btn btn-xs btn-white" title="{{ Lang::get('core.btn_edit') }}"><i class="fa fa-edit"></i></a>
						@endif
...
$(document).ready(function(){
	...
	$('.do-quick-search').click(function(){
		$('#SximoTable').attr('action','{{ URL::to("outbound/multisearch")}}');
		$('#SximoTable').submit();
	});
