...
class OutboundController extends BaseController {
	...
	public $module = 'outbound';
	static $per_page	= '10';
	
	public function __construct() {
		parent::__construct();
		$this->beforeFilter('csrf', array('on'=>'post'));
		$this->model = new Outbound();
	...
	public function getIndex()
	{
		...
		$this->layout->nest('content','outbound.index',$this->data)
	...
	function getAdd( $id = null)
	{
		...
		if($row)
		{
			...
			$this->data['row'] = $this->model->getColumnTable('outbound'); 
		}
		...
		$this->data['id'] = $id;
		$this->layout->nest('content','outbound.form',$this->data)->with('menus', $this->menus );	
	}
	...
	function getShow( $id = null)
	{
		...
		$this->data['id'] = $id;
		$this->data['access']		= $this->access;
		$this->layout->nest('content','outbound.view',$this->data)->with('menus', $this->menus );	
	}	
	...
	function postSave( $id =0)
	{
		...
		if ($validator->passes()) {
			$data = $this->validatePost('outbound');
			...
			$redirect = (!is_null(Input::get('apply')) ? 'outbound/add/'.$id.'?md='.$md.$trackUri :  'outbound?md='.$md.$trackUri );
			return Redirect::to($redirect)->with('messagetext',Lang::get('core.note_success'))->with('msgstatus','success');
			
		} else {
			$md = str_replace(" ","+",Input::get('md'));
			return Redirect::to('outbound/add/'.$id.'?md='.$md)->with('messagetext',Lang::get('core.note_error'))->with('msgstatus','error')
	...
	public function postDestroy()
	{
		...
		return Redirect::to('outbound?md='.Input::get('md'));
	}			
