...
class InboundController extends BaseController {
	...
	public $module = 'inbound';
	static $per_page	= '10';
	
	public function __construct() {
		parent::__construct();
		$this->beforeFilter('csrf', array('on'=>'post'));
		$this->model = new Inbound();
	...
	public function getIndex()
	{
		...
		$this->layout->nest('content','inbound.index',$this->data)
	...
	function getAdd( $id = null)
	{
		...
		if($row)
		{
			...
			$this->data['row'] = $this->model->getColumnTable('inbound'); 
		}
		...
		$this->data['id'] = $id;
		$this->layout->nest('content','inbound.form',$this->data)->with('menus', $this->menus );	
	}
	...
	function getShow( $id = null)
	{
		...
		$this->data['id'] = $id;
		$this->data['access']		= $this->access;
		$this->layout->nest('content','inbound.view',$this->data)->with('menus', $this->menus );	
	}	
	...
	function postSave( $id =0)
	{
		...
		if ($validator->passes()) {
			$data = $this->validatePost('inbound');
			...
			$redirect = (!is_null(Input::get('apply')) ? 'inbound/add/'.$id.'?md='.$md.$trackUri :  'inbound?md='.$md.$trackUri );
			return Redirect::to($redirect)->with('messagetext',Lang::get('core.note_success'))->with('msgstatus','success');
			
		} else {
			$md = str_replace(" ","+",Input::get('md'));
			return Redirect::to('inbound/add/'.$id.'?md='.$md)->with('messagetext',Lang::get('core.note_error'))->with('msgstatus','error')
	...
	public function postDestroy()
	{
		...
		return Redirect::to('inbound?md='.Input::get('md'));
	}			
