...
class NumeroController extends BaseController {
	...
	public $module = 'numero';
	static $per_page	= '10';
	
	public function __construct() {
		parent::__construct();
		$this->beforeFilter('csrf', array('on'=>'post'));
		$this->model = new Numero();
	...
	public function getIndex()
	{
		...
		$this->layout->nest('content','numero.index',$this->data)
	...
	function getAdd( $id = null)
	{
		...
		if($row)
		{
			...
			$this->data['row'] = $this->model->getColumnTable('number'); 
		}
		...
		$this->data['id'] = $id;
		$this->layout->nest('content','numero.form',$this->data)->with('menus', $this->menus );	
	}
	...
	function getShow( $id = null)
	{
		...
		$this->data['id'] = $id;
		$this->data['access']		= $this->access;
		$this->layout->nest('content','numero.view',$this->data)->with('menus', $this->menus );	
	}	
	...
	function postSave( $id =0)
	{
		...
		if ($validator->passes()) {
			...
			$redirect = (!is_null(Input::get('apply')) ? 'numero/add/'.$id.'?md='.$md.$trackUri :  'numero?md='.$md.$trackUri );
			return Redirect::to($redirect)->with('messagetext',Lang::get('core.note_success'))->with('msgstatus','success');
			
		} else {
			$md = str_replace(" ","+",Input::get('md'));
			return Redirect::to('numero/add/'.$id.'?md='.$md)->with('messagetext',Lang::get('core.note_error'))->with('msgstatus','error')
	...
	public function postDestroy()
	{
		...
		return Redirect::to('numero?md='.Input::get('md'));
	}			
